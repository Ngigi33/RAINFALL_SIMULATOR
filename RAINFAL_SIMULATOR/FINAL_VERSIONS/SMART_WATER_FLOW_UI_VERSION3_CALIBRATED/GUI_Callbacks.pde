...
public void START_AUTO() {
  if (system_mode == AUTOMATIC) {
    ...
    float input=float(cp5.get(Textfield.class, "L_MIN").getText());
    float container=(input/1.5682);
    //y = 1.5682x  and with x as subject  x=y/1.5682   


    
    /*_________PREVIOUS CALIBRATION___________*/
    //float container= (((1.0/918.0)* ( (sqrt((18360000.0 * input)-92392883.0))+2179.0))/2.0);
